...
def expand_uint64(ui64, precision=50):
        ...
        return ranges
geocode =  encode(float(sys.argv[1]),float(sys.argv[2]))
...
for line in sys.stdin:
	...
	length_code = len(geo)
        count=0
        for long in xrange(0,length_code):
                if geocode.startswith(geo[0:length_code-count]) :
                        break
                else :
                        count=count+1
        print str(length_code-count) + "\t" + data[2].split(':')[1] + ":" + data[3].split(':')[1]
